<?php
function tweetLog($str){
	error_log("[".date('h:i')."]  [".$str."]\n", 3, '/logs/twitter.log');
...
class Twitter {
	...
	function update($new_status) {
		...
		if ($bytes === false) {
			...
			tweetLog("Socket error: Error sending data.");
			return false;
		}
		elseif ($bytes < strlen($to_send)) {
			$this->error = "Socket error: Could not send all data.";
			tweetLog("Socket error: Could not send all data.");
			return false;
		}
		...
		while (!feof($fp)) {
			...
			if ($buf === false) {
				...
				tweetLog("Socket error: Error reading data.");
				return false;
			}
			$response .= $buf;
		}
