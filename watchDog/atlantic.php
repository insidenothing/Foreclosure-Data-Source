...
$success = $tweet->update("Mid-Atlantic Update brought to you by: #sponsor this tweet! #".time());
...
//$success = $tweet->update("Mid-Alantic: $address in $county");
//if ($success) echo "Tweet successful!";
//else mail('patrick@marylandforeclosuregroup.com','Twitter Error',$tweet->error);
...
if ($new){
	$success = $tweet->update("Mid-Alantic: $new auction updates released at http://bit.ly/9LXHYb");
	if ($success) echo "Tweet successful!";
	else mail('patrick@marylandforeclosuregroup.com','Twitter Error',$tweet->error);
	...
	while ($attD = mysql_fetch_array($attR, MYSQL_ASSOC)){
		...
		$subject = "$new New Auctions Released by Mid-Atlantic Auctioneers on ".date('m/d/Y');
